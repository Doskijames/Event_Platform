...
def _load_oauth_client_secret() -> Dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Tip: Base64 is safer in env vars. "
            f"Original error: {e}"
        ) from e
...
def _extract_client_id_secret(client_json: Dict[str, Any]) -> Tuple[str, str, str]:
    ...
    if not isinstance(block, dict):
        raise RuntimeError("OAuth client secret JSON missing 'installed' or 'web' section.")
...
def get_drive_service():
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Common causes: refresh token revoked, wrong client secret JSON, "
            "or OAuth consent screen still restricted (testing mode without test user). "
            f"Original error: {e}"
        ) from e
...
def drive_file_embed_url(file_id: str) -> str:
    # ✅ More reliable for <img src="..."> in many cases than drive.google.com/uc
    # (it often serves the raw bytes directly)
